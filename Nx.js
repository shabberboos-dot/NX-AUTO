// ==UserScript==
// @name         𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡 – OFFICIAL TRADING SYSTEM (v8.1 + RECOVERY MODE)
// @namespace    http://tampermonkey.net/
// @version      80.2.0
// @description  Custom NX XHAWON Pattern Logic | Robust Scanner | Smart Storage | Recovery Mode
// @author       𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        let st = { 
            ...
            maxTrades: 0,
            activeAI: 'NX SYSTEM',
            isRecovery: false // <--- রিকভারি মোড স্টেট
        };
        ...
        // ==========================================
        // 3. UPDATED PATTERN LOGIC WITH RECOVERY MODE
        // ==========================================
        const getSignal = (num) => num >= 5 ? 'BIG' : 'SMALL';

        // জিক-জ্যাক মার্কেট চেক (Big-Small-Big অথবা Small-Big-Small)
        function isZigZag(h) {
        ...
        const UserPatternLogic = (h) => {
            ...
            // ১. জিক-জ্যাক মার্কেট সনাক্ত হলে রিকভারি মোড অন হবে
            if (isZigZag(h)) {
                st.isRecovery = true;
                console.log(`⚠️ জিক-জ্যাক মার্কেট সনাক্ত (${h[2] >= 5 ? 'B':'S'}-${h[1] >= 5 ? 'B':'S'}-${h[0] >= 5 ? 'B':'S'}) → RECOVERY MODE ON`);
            }

            // ২. রিকভারি মোড অন থাকলে ট্রেডিং লজিক
            if (st.isRecovery) {
                const isTwoBig = (h[0] >= 5 && h[1] >= 5);
                const isTwoSmall = (h[0] < 5 && h[1] < 5);

                if (isTwoBig) {
                    console.log(`🔄 রিকভারি মোড: ২টি Big পাওয়া গেছে → ট্রেড: BIG`);
                    return 'BIG';
                } else if (isTwoSmall) {
                    console.log(`🔄 রিকভারি মোড: ২টি Small পাওয়া গেছে → ট্রেড: SMALL`);
                    return 'SMALL';
                } else {
                    console.log(`⏳ রিকভারি মোড: ২টি একই আসার অপেক্ষায়... → SKIP`);
                    return 'NO TRADE';
                }
            }

            // ৩. স্বাভাবিক ট্রেড লজিক (যখন রিকভারি মোড অফ থাকে)
            const trade = getSignal(h[0]);
            console.log(`📊 নরমাল ট্রেড: h[0]=${h[0]} → ${trade}`);
            return trade;
        };
            ...
            if (sDigRaw && sDigRaw.length >= 10) { 
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        if (isWin) {
                            st.stpIdx = 0;
                            st.isRecovery = false; // 🎉 উইন হলে রিকভারি মোড অফ করে নরমাল মোডে ফেরা
                            console.log("🎉 WIN! রিকভারি সফল → NORMAL MODE অ্যাক্টিভ।");
                        } else {
                            st.stpIdx = Math.min(st.stpIdx + 1, st.dynSeq.length - 1);
                            console.log("❌ LOSS! রিকভারি মোড চালু থাকবে।");
                        }
                    }
        ...
        goBtn.onclick = () => {
            ...
            st.tradesDone = 0;
            st.isRecovery = false; // ইন্জিন শুরু করার সময় রিকভারি মোড রিসেট
            if (st.timeLimit !== 'NO' && parseInt(st.timeLimit) > 0) {
