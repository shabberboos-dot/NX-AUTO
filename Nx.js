// ==UserScript==
// @name         𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡 – OFFICIAL TRADING SYSTEM (v8.1 + ADMIN CONTROL)
// @namespace    http://tampermonkey.net/
// @version      80.1.0
// @description  Custom NX XHAWON Pattern Logic | Robust Scanner | Smart Storage
// @author       𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ==========================================
        // 3. PATTERN LOGIC (Recovery Mode: No Duplicate Check)
        // ==========================================
        const getSignal = (num) => num >= 5 ? 'BIG' : 'SMALL';

        function checkAlternating(h) {
            const last4 = h.slice(0, 4);
            for (let i = 1; i < last4.length; i++) {
                const prevBig = last4[i-1] >= 5;
                const currBig = last4[i] >= 5;
                if (prevBig === currBig) return false;
            }
            return true;
        }
        ...
        const UserPatternLogic = (h) => {
            ...
            const currentNum = h[0];

            // ---------- RECOVERY MODE (শুধু যোগফল, ডুপ্লিকেট চেক নেই) ----------
            if (st.recoveryMode) {
                // 1. Zero check (শুধু ০ চেক)
                if (currentNum === 0 || h[9] === 0) {
                    console.log(`🚫 Recovery: 0 involved → NO TRADE`);
                    return 'NO TRADE';
                }
                // 2. যোগফল (h[0] + h[9])
                const sum = currentNum + h[9];
                // 3. যোগফল অনুযায়ী সিদ্ধান্ত
                let trade = 'NO TRADE';
                if (sum >= 1 && sum <= 4) {
                    trade = 'SMALL';
                } else if (sum >= 5 && sum <= 9) {
                ...
                console.log(`♻️ Recovery Mode → h[0]=${currentNum}, h[9]=${h[9]}, যোগ=${sum} → ট্রেড: ${trade}`);
                return trade;
            }

            // ---------- ALTERNATING SKIP ----------
            const isAlternating = checkAlternating(h);
            if (isAlternating) {
                st.recoveryMode = true;
                console.log(`⏭️ Alternating → SKIP, Recovery Mode চালু`);
                return 'NO TRADE';
            }

            // ---------- DEFAULT MODE ----------
            const trade = getSignal(h[0]);
            console.log(`📊 Default Mode → h[0]=${h[0]} → ট্রেড: ${trade}`);
            return trade;
        };

        // ==========================================
        // 4. UI & SCANNER (Standard - Unchanged)
        // ==========================================
        let dTimeLeft = 30;
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title" ondblclick="window._toggleML()">${uF('NX XHAWON')}</span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        window._toggleML = function() {
            ...
            if(st.mlActive) {
                ...
                titleEl.innerText = uF('NX XHAWON (ML)');
            } else {
                titleEl.style.color = '';
                titleEl.innerText = uF('NX XHAWON');
            }
        };
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-cyan">NX SYSTEM</span></div>`;
        ...
        infBx.innerHTML = aiRow +
                            ...
                            `<div style="display:flex;justify-content:space-between;border-top:1px solid #333;margin-top:2px;padding-top:2px;"><span style="color:#666;font-size:8px;">${uF('MODE:')}</span><span id="ui-mode" style="color:#0ff;font-size:8px;">DEFAULT</span></div>`;
        ...
        const loopTask = () => {
            ...
            if (uMode) uMode.innerText = st.recoveryMode ? 'RECOVERY' : 'DEFAULT';
            ...
            if (sDigRaw && sDigRaw.length >= 10) { 
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        // Recovery উইন হলে ডিফল্টে ফেরত
                        if (st.recoveryMode && isWin) {
                            st.recoveryMode = false;
                            console.log(`✅ Recovery উইন! → Default Mode ফিরে গেলাম`);
                        }
